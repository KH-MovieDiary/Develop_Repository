        ...
        <div class="content1-body">
            <div class="profile-area">
                <div class="profile-img">
                    <img src="${loginUser.picture != null ? loginUser.picture : '/resources/img/default_profile.png'}">
                </div>
        ...
        <c:choose>
          ...
          <c:otherwise>
            <c:forEach var="cmt" items="${myCommentList}">
              <!-- ✅ 카드 전체 클릭 이동용 reviewId -->
              <div class="item_content" data-review-id="${cmt.reviewId}">
...
$(function () {
  ...
  /* =========================
     (2) 댓글 삭제 모달 (삭제 버튼 클릭 -> 팝업 띄우기)
     ========================= */
  $(document).on("click", ".btn-del-comment", function (e) {
    e.stopPropagation(); // ✅ 카드 클릭 이동 이벤트 차단(삭제는 항상 팝업)
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        $("#commentDeleteModal").modal("show"); // ✅ 기존처럼 팝업 유지
      },
      error: function (xhr) {
...
/* =========================
   ✅ 위시리스트 영화 클릭 -> openModal(tmdbId)
   ========================= */
document.addEventListener("click", function(e){
  var card = e.target.closest(".content2 .item_content");
  if(card){
    var tmdbId = card.getAttribute("data-tmdb-id");
    if(tmdbId && typeof openModal === "function"){
      openModal(tmdbId);
    }
    return;
  }
...
/* =========================
   ✅ 내가 쓴 댓글 카드 클릭 -> 해당 감상평 상세로 이동
   - 삭제 버튼 클릭은 제외 (팝업 유지)
   - 링크 클릭은 기본 동작 유지
   ========================= */
document.addEventListener("click", function(e){
  var commentCard = e.target.closest(".content4 .item_content");
  if(!commentCard) return;

  // ✅ 삭제 버튼이면 카드 이동 절대 X
  if(e.target.closest(".btn-del-comment")) return;

  // ✅ 모달 내부 클릭이면 이동 X
  if(e.target.closest("#commentDeleteModal")) return;

  // ✅ 제목 링크 클릭이면 기본 링크 이동
  if(e.target.closest("a")) return;

  var rid = commentCard.getAttribute("data-review-id");
  if(!rid) return;

  location.href = "${pageContext.request.contextPath}/detail.review?rno=" + rid;
});
